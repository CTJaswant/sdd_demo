...
- **Audit persistence — narrowed per key decision:** the `EligibilityCheck` entity stores **only** `CorrelationId` (Guid), `Status` (string: `ELIGIBLE`/`INELIGIBLE`/`ERROR`), `CheckedAt` (UTC timestamp), and `DataSource` (string, `"LOCAL_DB"` for Phase 1 — reserved for `"PAYER_API"` in Phase 2). It does **not** store `patientId`, `healthPlanId`, `errorCode`, or any name/demographic field.
  - **This is narrower than spec.md OQ-02's resolved schema**, which had proposed including `patient_id`/`health_plan_id` in `eligibility_checks` for traceability. This design note's schema takes precedence per this task's explicit instruction; spec.md §9 OQ-02 and §6 Constraint 9 should be updated to match (see Open Questions).
...
1. **Audit schema mismatch with spec.md.** This design's audit record (`correlationId`, `status`, `checkedAt`, `dataSource`) is narrower than spec.md §9 OQ-02's resolved schema (which included `patient_id`/`health_plan_id`). Spec.md §9 OQ-02 and §6 Constraint 9 should be updated to match this design before PLAN.md/TASKS.md are written. [NON-BLOCKING for this design note; BLOCKING for spec.md consistency.]
